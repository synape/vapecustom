...
local tp = {["Enabled"] = false}
tp = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
["Name"] = "tp",
["Function"] = function(callback)
if callback then
 local ohString1 = "dash"
    ...
    tp["ToggleButton"](false)
    createwarning("Don't move", "Don't move", 3)
end
end
})
...
local AnticheatDisabler = COB("World", {
    ...
    ["Function"] = function(callback)
        if callback then
            ...
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(asd)+Vector3.new(0,100,0)
            ...
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame + game.Players.LocalPlayer.Character.Humanoid.MoveDirection * 0.2
            else
            workspace.Gravity = 192.6
	...
	local ColletEmeralds = COB("Blatant", {
    ["Name"] = "ColletEmeralds",
    ["Function"] = function(callback)
        if callback then
    ...
    ["Default"] = false,
    ["HoverText"] = "Collets Emeralds"
})

	local TpAURA = COB("Blatant", {
    ["Name"] = "TpAURA",
    ["Function"] = function(callback)
        if callback then
...
local swordexploit = COB("Blatant", {
    ["Name"] = "SwordExploit",
    ["Function"] = function(callback)
        if callback then
...
local theyes = {["Enabled"] = false}
    theyes = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ...
        ["HoverText"] = "Enable when TNT explodes then it should make you go 90 speed for about 2 seconds. (Does not work with speed)",
        ["Function"] = function(callback)
            if callback then
